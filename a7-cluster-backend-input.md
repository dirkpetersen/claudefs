# claudefs-gateway Phase 2: ClusterVfsBackend — Wire NFS to A2/A4 Transport
...
Create a new file `src/cluster_backend.rs` that implements the `VfsBackend` trait
(defined in `src/nfs.rs`) using the `GatewayConnPool` for backend connections to
ClaudeFS metadata servers (A2) via A4 transport.

This is the integration layer that connects the NFS gateway to the real distributed
file system. When A2/A4 are fully ready, only this module needs to be updated —
the rest of the NFS gateway stays the same.

## Context: Existing Types

From `src/nfs.rs`, the `VfsBackend` trait has these methods:
```rust
...
From `src/protocol.rs`, these types are defined:
- `FileHandle3` — NFS file handle (opaque bytes identifying a file)
- `Fattr3` — file attributes (size, mode, uid, gid, timestamps, etc.)
- `LookupResult` — result of filename lookup (file handle + attributes)
- `ReadDirResult` — directory listing result
- `FsStatResult` — filesystem statistics
- `FsInfoResult` — filesystem capability info
- `PathConfResult` — POSIX path configuration
- `Nfstime3` — NFS timestamp (seconds + nanoseconds)
- `Ftype3` — file type enum (Reg, Dir, Lnk, etc.)
- `Entry3` — directory entry (filename + inode cookie)

From `src/error.rs`, `GatewayError` and `type Result<T> = std::result::Result<T, GatewayError>`.

From `src/gateway_conn_pool.rs`, `GatewayConnPool` is available.

## Task: Create `src/cluster_backend.rs`

Write a new file with a `ClusterVfsBackend` that:

1. Holds a `Arc<Mutex<GatewayConnPool>>` for backend connections
2. Implements all `VfsBackend` methods as **stubs that return appropriate errors** until
   the A2/A4 RPC protocol is finalized
3. Has a constructor `ClusterVfsBackend::new(nodes: Vec<NodeInfo>, config: ConnPoolConfig) -> Self`
4. Includes a `ClusterStats` struct tracking: total_rpc_calls, successful_rpcs, failed_rpcs,
   backend_bytes_read, backend_bytes_written
5. Has `fn stats(&self) -> ClusterStats` to read current statistics
6. Uses `tracing::debug!` and `tracing::warn!` for operation logging

The stubs should return `GatewayError::NotImplemented { feature: "...".into() }` for
operations that are not yet wired to A2, but with proper span instrumentation so
real operations can be tracked once integrated.

Here's the complete implementation to write:
...
use crate::gateway_conn_pool::{ConnPoolConfig, GatewayConnPool, NodeInfo};
...
impl VfsBackend for ClusterVfsBackend {
    ...
    #[instrument(skip(self), fields(cluster = %self.cluster_name))]
    fn read(&self, fh: &FileHandle3, offset: u64, count: u32) -> Result<(Vec<u8>, bool)> {
        debug!(fh_len = fh.data.len(), offset = offset, count = count, "read");
        Err(self.not_implemented("read"))
    }

    #[instrument(skip(self, data), fields(cluster = %self.cluster_name))]
    fn write(&self, fh: &FileHandle3, offset: u64, data: &[u8]) -> Result<u32> {
        debug!(fh_len = fh.data.len(), offset = offset, bytes = data.len(), "write");
        Err(self.not_implemented("write"))
    }

    #[instrument(skip(self), fields(cluster = %self.cluster_name))]
    fn readdir(&self, dir_fh: &FileHandle3, cookie: u64, count: u32) -> Result<ReadDirResult> {
        debug!(dir_fh_len = dir_fh.data.len(), cookie = cookie, count = count, "readdir");
        Err(self.not_implemented("readdir"))
    }

    #[instrument(skip(self), fields(cluster = %self.cluster_name))]
    fn mkdir(&self, dir_fh: &FileHandle3, name: &str, mode: u32) -> Result<(FileHandle3, Fattr3)> {
        debug!(dir_fh_len = dir_fh.data.len(), name = name, mode = mode, "mkdir");
        Err(self.not_implemented("mkdir"))
    }

    #[instrument(skip(self), fields(cluster = %self.cluster_name))]
    fn create(&self, dir_fh: &FileHandle3, name: &str, mode: u32) -> Result<(FileHandle3, Fattr3)> {
        debug!(dir_fh_len = dir_fh.data.len(), name = name, mode = mode, "create");
    ...
    fn symlink(
        ...
        debug!(dir_fh_len = dir_fh.data.len(), name = name, target = target, "symlink");
    ...
    #[instrument(skip(self), fields(cluster = %self.cluster_name))]
    fn access(&self, fh: &FileHandle3, uid: u32, gid: u32, access_bits: u32) -> Result<u32> {
        debug!(fh_len = fh.data.len(), uid = uid, gid = gid, bits = access_bits, "access");
...
mod tests {
    ...
    fn test_getattr_returns_not_implemented() {
        let backend = make_backend();
        let fh = FileHandle3 { data: vec![1, 2, 3, 4] };
        let result = backend.getattr(&fh);
        assert!(result.is_err());
        match result.unwrap_err() {
            GatewayError::NotImplemented { feature } => {
                assert!(feature.contains("getattr"), "feature should mention getattr, got: {}", feature);
            }
    ...
    fn test_all_ops_return_not_implemented() {
        let backend = make_backend();
        let fh = FileHandle3 { data: vec![0, 0, 0, 1] };
        let dir_fh = FileHandle3 { data: vec![0, 0, 0, 2] };
...
## Task 2: Register the module in `lib.rs`

Add after the existing `/// Async TCP listener for NFSv3 RPC connections.` line:
...
1. **Check `NodeInfo` struct first** — look in `gateway_conn_pool.rs` to see if `NodeInfo`
   exists and what fields it has. Adjust the `ClusterVfsBackend::new()` signature accordingly.
2. **Check `FileHandle3` struct** — look in `protocol.rs` to see if it has a `data: Vec<u8>`
   field or different structure. Adjust test code accordingly.
3. **Do NOT change any existing code** except adding the module declaration to `lib.rs`.
4. **All new public items must have doc comments** (crate has `#![warn(missing_docs)]`).
5. After writing the files, run `cargo test -p claudefs-gateway` — all tests must pass.

The goal is 1135+ tests passing with 0 warnings and 0 errors.
