...
use claudefs_reduce::key_manager::{
    KeyManager, KeyManagerConfig, DataKey, WrappedKey,
};
...
fn finding_32_nonce_entropy_distribution() {
    ...
    for i in 0..12 {
        for count in &byte_counts[i] {
            ...
            assert!(deviation < 0.1, "Byte {} has bias: {} vs expected {}", i, count, expected);
        }
    }
}
...
fn finding_32_nonce_birthday_bound() {
    ...
    assert!(birthday_bound > 1_000_000, "Birthday bound for 96-bit nonce should be > 1M");
    assert!(collision_prob_10k < 0.01, "Collision probability for 10k nonces should be < 1%");
}
...
fn finding_33_hkdf_context_separation() {
    ...
    assert_ne!(key1.0, key2.0, "Different inputs should produce different keys");
}
...
fn finding_34_key_pruning_data_loss() {
    ...
    assert!(result.is_err(), "Old DEK should be inaccessible after pruning");
}
...
fn finding_34_key_version_sequence() {
    ...
    assert!(v0 < v1 && v1 < v2, "Versions should increment monotonically");
}
...
fn finding_35_ciphertext_portability() {
    ...
    assert_eq!(dec, plaintext, "Same ciphertext should decrypt with same key");
}
...
fn prop_key_derivation_collision_resistance() {
    ...
    proptest! {
        #[test]
        fn prop_no_collision((h1, h2): ([u8; 32], [u8; 32])) {
            if h1 != h2 {
...
fn key_manager_initial_version_zero() {
    let mut km = KeyManager::with_initial_key(KeyManagerConfig::default(), EncryptionKey([0xAA; 32]));
