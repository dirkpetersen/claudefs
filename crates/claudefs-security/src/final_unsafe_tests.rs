...
use claudefs_transport::zerocopy::{RegionPool, ZeroCopyConfig, MemoryRegion};
use claudefs_repl::batch_auth::{BatchAuthenticator, BatchAuthKey};
use std::sync::Arc;
use std::thread;
...
fn finding_50_batch_auth_wrong_key() {
    ...
    assert!(!auth2.verify(data, &sig), "Different keys should not verify");
}
...
fn finding_50_batch_auth_tampered_data() {
    ...
    assert!(!auth.verify(&data, &signature), "Tampered data should not verify");
}
...
fn finding_51_zerocopy_alignment_check() {
    for alignment in &[512, 1024, 4096, 8192] {
        ...
        if let Some(mut region) = pool.acquire() {
            ...
            assert_eq!(ptr % alignment, 0, "Region should be aligned to {}", alignment);
        }
    }
}
...
fn zerocopy_preregister_capped() {
    ...
    assert!(pool.total() <= config.max_regions, "Preregister should be capped to max_regions");
}
...
fn batch_auth_empty_data() {
    ...
    assert!(auth.verify(empty, &sig), "Empty data should sign and verify");
}
...
fn batch_auth_large_data() {
    ...
    assert!(auth.verify(&large_data, &sig), "Large data should sign and verify");
}
...
fn prop_region_pool_invariant() {
    ...
    proptest! {
        #[test]
        fn prop_pool_invariant((acquire_count, release_count): (usize, usize)) in (0..50usize, 0..50usize) {
            let config = ZeroCopyConfig {
                region_size: 4096,
                max_regions: 100,
                alignment: 4096,
