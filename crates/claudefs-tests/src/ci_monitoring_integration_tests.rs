...
mod alert_rule_validation {
    ...
    fn test_alert_cost_threshold_correct() {
        ...
        let empty_vec: Vec<serde_yaml::Value> = Vec::new();
        let rules = cost_group["rules"].as_sequence().unwrap_or(&empty_vec);
...
mod alertmanager_config {
    ...
    fn test_alertmanager_routes_cover_all_severity_levels() {
        ...
        let empty_vec: Vec<serde_yaml::Value> = Vec::new();
        let routes = route["routes"].as_sequence().unwrap_or(&empty_vec);
        for r in routes {
            let match_value = &r["match"];
            let severity = match match_value {
                serde_yaml::Value::Mapping(m) => {
                    m.get(&serde_yaml::Value::String("severity".to_string()))
                }
                _ => None,
            };
            if let Some(sev) = severity {
                let sev_str = sev.as_str().unwrap_or("");
                if sev_str == "critical" {
                    has_critical = true;
